...
LAPS_COLUMN_NAMES = ['number', 'avg_cadence', 'avg_combined_pedal_smoothness', 'avg_grade',
                     'avg_heart_rate', 'avg_left_pedal_smoothness', 'avg_left_torque_effectiveness',
                     'avg_neg_grade', 'avg_pos_grade', 'avg_power', 'avg_right_pedal_smoothness',
                     'avg_right_torque_effectiveness', 'avg_speed', 'avg_temperature', 'end_position_lat',
                     'end_position_long', 'enhanced_avg_speed', 'enhanced_max_altitude', 'enhanced_max_speed',
                     'enhanced_min_altitude', 'event', 'event_group', 'event_type', 'intensity', 'lap_trigger',
                     'left_right_balance', 'max_altitude', 'max_cadence', 'max_heart_rate', 'max_neg_grade',
                     'max_pos_grade', 'max_power', 'max_speed', 'max_temperature', 'message_index',
                     'min_altitude', 'sport', 'start_position_lat', 'start_position_long', 'start_time',
                     'sub_sport', 'timestamp', 'total_ascent', 'total_calories', 'total_descent',
                     'total_distance', 'total_elapsed_time', 'total_fat_calories', 'total_moving_time',
...
def get_fit_lap_data(frame: fitdecode.records.FitDataMessage) -> Dict[str, Union[float, datetime, timedelta, int]]:
    ...
    # print([f.name for f in frame.fields])
...
def get_dataframes(fname: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    laps_df = pd.concat([pd.DataFrame({"FileName": [filename]*len(laps_data)}),
                         pd.DataFrame(laps_data, columns=LAPS_COLUMN_NAMES)], axis = 1)

    laps_df.set_index('number', inplace=True)
    points_df = pd.concat([pd.DataFrame({"FileName": [filename]*len(points_data)}),
                         pd.DataFrame(points_data, columns=POINTS_COLUMN_NAMES)], axis = 1)
